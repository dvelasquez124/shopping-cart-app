...
router.put('/:id', ensureAdmin, async (req, res) => {
    try {
      ...
      // if body is missing, treat as empty object (prevents "in undefined" error)
      ...
      // pick only allowed keys if they are present on the body
      ...
      if (Object.keys(updates).length === 0) {
        return res.status(400).json({ error: 'No valid fields supplied to update.' });
      }
  
      // Coerce and validate numbers (forms send strings)
      if ('price' in updates) {
      ...
      if ('quantityInStock' in updates) {
        updates.quantityInStock = parseInt(updates.quantityInStock, 10);
